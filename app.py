...
def fetch_stock_data(ticker_symbol):
    ...
    profiles = {
        "AAPL": {
            ...
            "shares_outstanding": 14_690_000_000,
            "cash": 73_000_000_000,
        ...
        "MSTR": {
            ...
            "shares_outstanding": 356_000_000,
            "cash": 50_000_000,
...
def calculate_2stage_dcf(rev_0, margin_0, target_margin, growth_high, sc_ratio, wacc_high, terminal_growth=0.03, terminal_wacc=0.075, tax_rate=0.21, return_details=False):
    ...
    operating_assets_value = sum(pvs) + pv_terminal
...
strategic_treasury = st.sidebar.slider(
    ...
    0.1,
    help="Explicitly isolate long-term strategic holdings from operating assets."
)
...
intrinsic_value_per_share = max(0.0, equity_value / max(stock_data["shares_outstanding"], 1))

tv_contribution = (dcf_result["pv_terminal_value"] / operating_value) * 100 if operating_value > 0 else 0
...
with col_right:
    ...
    price_pct_diff = ((intrinsic_value_per_share - stock_data['current_price']) / stock_data['current_price']) * 100 if stock_data['current_price'] else 0
    ...
    if tv_contribution > 80.0:
        ...
        <div class='warning-box'>
            ...
            Present value of terminal value is <strong>{tv_contribution:.1f}%</strong> of operating assets.
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown(f"""
        <div class='info-box'>
            ...
            Terminal value is <strong>{tv_contribution:.1f}%</strong> of operating assets.
        </div>
        """, unsafe_allow_html=True)
...
with t_col2:
    ...
    if abs(growth_rate - stock_data["revenue_growth_1y"]) > 0.25:
        warnings.append(f"• Target growth ({growth_rate*100:.0f}%) is far from historical growth ({stock_data['revenue_growth_1y']*100:.0f}%).")
